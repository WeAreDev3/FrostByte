...
Enemy = Character.extend({
  ...
  draw: function (context, scale) {
    ...
    if (this.hitPoints <= 0) {
      context.fillStyle = 'rgba(' + (this.alpha > 0.6 ? 95 : 43) + ',149,238,' + (this.alpha > 0.3 ? 1 : this.alpha + 0.3) + ')'
      context.beginPath()
      context.arc(x, y, size * (this.alpha > 0.7 ? 0.3 : (1 - this.alpha)), 0, 2 * Math.PI, false)
      context.fill()
      context.closePath()
    }
